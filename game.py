...
class Game:
  ...
  def run(self):
    ...
    self.winner_screen_surface = input_font.render("YOU ARE NOW THE WORLD'S GREATEST GEODLER!",True,"Black")
      ...
      for event in pygame.event.get():
        ...
        if self.guess_count < 10:
          if self.game_active:
                    ...
                    if self.output_list == "invalid":
                      ...
                      for country in self.comparison.country_info_list:
                        if country.name == self.user_text:
                          ...
                          self.box_row_list[self.guess_count][3].text = str(country.area)
                          self.box_row_list[self.guess_count][4].text = str(country.pop)
            ...
            self.screen.blit(self.background_surface,(0,0))
            self.screen.blit(self.winner_screen_surface,(200,400))
            self.screen.blit(self.instructions_surface,(150,550))
            self.screen.blit(self.clout_surface,(400,-40))
        else: #End Lose Screen
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
              self.run()

            self.screen.blit(self.background_surface,(0,0))
            self.screen.blit(self.loser_screen_surface,(200,400))
            self.screen.blit(self.instructions_surface,(150,550))
            self.game_active = False
